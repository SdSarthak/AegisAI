...
# 1. Split first — before any vectorization
X_train_raw, X_test_raw, y_train, y_test = train_test_split(
    dt_data['sentence'], dt_data['label'], test_size=0.30, random_state=5
)
...
X_train = cv.fit_transform(X_train_raw)                 ## Transform the text using our object.

# 3. Apply the already-fitted CountVectorizer to test data (no re-fitting)
