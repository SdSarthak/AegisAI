...
export default function Guard() {
  ...
  const metricsPayload = result?.execution_metrics
    ? JSON.stringify(result.execution_metrics, null, 2)
    : ''
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-semibold text-gray-900">
              {result.is_injection ? 'Injection Detected' : 'Prompt Safe'}
            </h2>
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
              result.is_injection ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
            }`}>
