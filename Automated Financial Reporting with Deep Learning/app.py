...
# 1. Split first — before any scaling
from sklearn.model_selection import train_test_split

X_train, X_test = train_test_split(
    df.drop('Date', axis=1), test_size=0.2, random_state=42
)
...
# 3. Apply the already-fitted scaler to test data (no re-fitting)
X_test_scaled = scaler.transform(X_test)         # transform only — no fit

# Transform the full data for prediction/display without re-fitting
data_scaled = scaler.transform(df.drop('Date', axis=1))
