...
Below are common setup and runtime issues contributors may encounter while working with AegisAI.

### 1. Docker Compose Startup Failures

#### Problem
Docker containers fail to start or exit immediately.
...
### 2. Backend `.env` Configuration Issues

#### Problem
Backend fails to start because required environment variables are missing.

#### Solution

Ensure the `.env` file exists inside the `backend/` directory.

```bash
cp backend/.env.example backend/.env
```

Verify that important variables are configured:

```env
SECRET_KEY=your_secret_key
LLM_API_KEY=your_api_key
DATABASE_URL=postgresql://user:password@localhost:5432/aegisai
```
...
Verify that database credentials inside `.env` are correct.
...
#### Problem
Ports `5173`, `8000`, or `5432` are already in use.
...
You may also change the configured port if needed.

---

### 5. Ollama Connectivity Issues

#### Problem
Backend cannot connect to Ollama service.
...
Ensure the following variables are correctly configured in `backend/.env`:
...
Upgrade `pip` before installing backend dependencies:
...
If installation issues persist, clear cache and reinstall dependencies.

---

### 7. Python or Node.js Version Mismatch

#### Problem
Project behaves unexpectedly because of unsupported runtime versions.
