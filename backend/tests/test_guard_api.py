...
def test_update_guard_config_success(authenticated_client: TestClient):
    ...
    assert get_resp.json()["sanitization_level"] == "high"
...
def test_update_guard_config_validation(
...
def test_scan_prompt_rejects_oversized_prompt(authenticated_client: TestClient):
    oversized_prompt = "a" * (settings.GUARD_MAX_PROMPT_LENGTH + 1)
    ...
    assert any("String should have at most" in error["msg"] for error in detail)
...
def test_scan_prompt_rate_limit(authenticated_client: TestClient):
    ...
    from app.api.v1.guard import _RATE_LIMIT_REQUESTS, _scan_attempts_by_user
...
def test_bulk_scan_rejects_oversized_prompt(authenticated_client: TestClient):
    oversized_prompt = "a" * (settings.GUARD_MAX_PROMPT_LENGTH + 1)

    response = authenticated_client.post(
        "/api/v1/guard/scan/batch",
        json={"prompts": ["safe prompt", oversized_prompt]},
    )
