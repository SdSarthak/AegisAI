...
@pytest.mark.asyncio
async def test_deliver_webhook_sends_correct_headers(db_session, test_user):
    ...
    with respx.mock(assert_all_called=True) as router:
        ...
        await deliver_webhook(db_session, test_user.id, event, payload)
...
@pytest.mark.asyncio
async def test_deliver_webhook_ignores_failed_delivery(db_session, test_user):
    event = "guard_block"
    config = WebhookConfig(
        user_id=test_user.id,
    ...
    with respx.mock(assert_all_called=True) as router:
        ...
        await deliver_webhook(db_session, test_user.id, event, {"ok": True})
