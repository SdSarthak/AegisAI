...
class TestCSRFMiddleware:
    ...
    def test_safe_methods_do_not_require_csrf(self, plain_client, auth_headers):
        """GET endpoints are exempt from CSRF."""
        resp = plain_client.get("/health")
        assert resp.status_code == 200
        resp2 = plain_client.get("/api/v1/auth/me", headers=auth_headers)
        assert resp2.status_code == 200

    def test_badge_endpoints_are_exempt(self, plain_client):
        """All /badge/* paths are exempt from CSRF."""
        resp = plain_client.post("/badge/verify", json={})
        assert resp.status_code != 403, resp.text

    def test_csrf_token_endpoint_itself_is_exempt(self, plain_client):
        """POST to /csrf-token does not trigger a CSRF loop."""
        resp = plain_client.post("/api/v1/auth/csrf-token")
        assert resp.status_code != 403, resp.text
    ...
    def test_exempt_path_with_csrf_mismatch_still_passes(self, plain_client):
