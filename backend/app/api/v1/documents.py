...
# NOTE: /generate must be declared before /{document_id} so FastAPI does not
# treat the literal string "generate" as an integer document_id and return 422.
# FIX: added status_code=201 — creating a resource must return 201 Created,
# not 200 OK. The tests (and REST semantics) both require this.
@router.post(
    "/generate",
    response_model=DocumentResponse,
    status_code=status.HTTP_201_CREATED,   # ← THE FIX
)
def generate_document(
    ...
    # Fetch AI system owned by this user
    ai_system = (
        db.query(AISystem)
        .filter(
    ...
    # Try LLM-powered generation; fall back to template on any failure
    try:
...
def export_document_pdf(
    ...
    story.append(
        Paragraph(f"<b>Document Type:</b> {document.document_type.value}", metadata_style)
    )
    story.append(
        Paragraph(f"<b>Status:</b> {document.status.value}", metadata_style)
    )
    story.append(
        Paragraph(
            f"<b>Created:</b> {document.created_at.strftime('%Y-%m-%d %H:%M:%S')}",
            metadata_style,
        )
    )
    ...
    for line in document.content.split("\n"):
        if not line.strip():
            ...
            h1 = ParagraphStyle(
                "H1", parent=styles["Heading1"], fontSize=16, spaceAfter=12, spaceBefore=12
            )
            story.append(Paragraph(line[2:], h1))
        elif line.startswith("## "):
            h2 = ParagraphStyle(
                "H2", parent=styles["Heading2"], fontSize=13, spaceAfter=10, spaceBefore=10
            )
            story.append(Paragraph(line[3:], h2))
        elif line.startswith("### "):
            h3 = ParagraphStyle(
                "H3", parent=styles["Heading3"], fontSize=11, spaceAfter=8, spaceBefore=8
            )
            story.append(Paragraph(line[4:], h3))
        elif line.startswith("- "):
            story.append(Paragraph("• " + line[2:], body_style))
        else:
            processed = re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", line.strip())
