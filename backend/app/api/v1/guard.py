...
def get_guard_history(
    ...
    Args:
        ...
        decision: Filter logs by decision (allow, sanitize, block).
        intent: Filter logs by intent (benign, suspicious, malicious).
        start_date: Filter logs scanned after this date.
        end_date: Filter logs scanned before this date.
        db: Database session used to query scan history.
        current_user: Authenticated user whose history is requested.

    Returns:
        CursorPaginatedResponse containing the user's scan history and next cursor.
    """
    ...
    return CursorPaginatedResponse(items=logs, limit=limit, next_cursor=next_cursor)
