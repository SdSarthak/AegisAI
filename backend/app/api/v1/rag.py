...
def query_knowledge_base(
    ...
    try:
        ...
        # ------------------------------------------------------
        # Persist RAGFeedback row so /feedback can look it up
        # by answer_id and the source_chunks are retained.
        # ------------------------------------------------------
...
def rag_feedback(
    ...
    try:
        ...
        if not feedback:
            # Row not found — create a stub so the vote is never lost.
            feedback = RAGFeedback(
        ...
        db.commit()
        # -------------------------------------------------------
        # FIX: removed db.refresh(feedback) that was here.
        #
        # db.refresh() after commit() raises an exception in some
        # SQLAlchemy / SQLite configurations (e.g. DetachedInstanceError).
        # The bare `except Exception: db.rollback()` below was catching
        # that silently, rolling back the thumbs_down increment, so the
        # row stayed at thumbs_down=0 and never appeared in the
        # low-quality-chunks results.
        #
        # db.refresh() is not needed here: the only value used from
        # `feedback` after this point is feedback.id, which is already
        # in memory from the earlier .first() query.
        # -------------------------------------------------------
